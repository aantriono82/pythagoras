document.addEventListener('DOMContentLoaded', () => {
    ...
    let calculationFrame = null;
    let calculationRequest = 0;
    const triplesCache = new Map();
    ...
    function findTriples(limit, primitiveOnly) {
        const cacheKey = `${limit}:${primitiveOnly}`;
        if (triplesCache.has(cacheKey)) return triplesCache.get(cacheKey);
        ...
        triples.sort((x, y) => x[2] - y[2] || x[0] - y[0] || x[1] - y[1]);
        triplesCache.set(cacheKey, triples);
        return triples;
    }
    ...
    function calculateTriples() {
        ...
        if (calculationFrame !== null) window.cancelAnimationFrame(calculationFrame);
    ...
    shareButtons.addEventListener('click', event => {
        ...
        if (button) shareContent(button.dataset.platform);
    });
